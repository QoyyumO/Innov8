...
Judges dry-run after those seven steps (VERIFY / step-up, `/security` queue, Chioma's portal, `/facilities`): root `README.md`.
...
- **INN-36** patient discovery — `convex/patients.ts`, `/patients`, `/patients/[publicId]`. Demo step 2 works: identity + record existence only, audited. Public IDs are case-insensitive (`trim().toUpperCase()`, INN-82). Every `searchPatients` call that reaches an index writes `PatientSearched`, including zero hits (`resultCount: 0`) (INN-56). A query rejected before any lookup — empty, or a name prefix under `NAME_PREFIX_MIN_LENGTH` — is not a search and is not audited; `isSearchableQuery` in `convex/lib/searchLimits.ts` is the single definition of that line, shared by the service, the mutation, and the search form. Opening `/patients/[publicId]` writes one `PatientDiscovered` per session and publicId, including misses (INN-69).
...
- **INN-37** access requests — `convex/accessRequests.ts` (`createAccessRequest`, `listMyAccessRequests`, `getAccessRequest`) + `convex/lib/services/accessControlService.ts`. Clinicians only; source facility from `users.facilityId` (falls back to `hospital` name), target from the patient's record index; stores request + decision; audits `AccessRequested` and `AccessAllowed` / `AccessChallenged` / `AccessBlocked`. Pharmacists may request medications and allergies; laboratory staff diagnoses and lab results; doctors and nurses keep the Track C types plus lab results (INN-79 / INN-80). UI: `/requests`, `/requests/new`, `/requests/[requestId]`. Demo steps 3–4 work. Scores one patient per request; `recordCount` is never a client argument.
- **INN-40** authorised summary — `convex/records.ts` `viewAuthorisedSummary` (mutation, so it can audit) + `convex/lib/services/recordExchangeService.ts`. Only the requester; only after ALLOW or a live emergency grant on the same request; only the requested record types; only the target facility's summary (`conditions` never returned). Lab results exist on the index until ALLOW (INN-80). Audits `RecordViewed`. UI: **View authorised records** on `/requests/[requestId]`. Demo step 5 works.
...
- **INN-45** consent — `consents` table + `convex/lib/services/consentService.ts` + `convex/consents.ts` (`getConsentStatus`, `recordPatientConsent`, `revokePatientConsent`, `listConsents`, plus `listMyConsents` / `grantMyConsent` / `revokeMyConsent` in INN-77). Single-patient, cross-facility requests with purpose other than `emergency` need a live consent for the requesting facility; `evaluateConsent` feeds `RiskInput.consent` (missing → +35, at least VERIFY; reason in the decision; `factors.consent`). Step-up is refused until consent exists. Clinicians record consent (note, 30 days, `ConsentRecorded`); security officers / admins in scope revoke (`ConsentRevoked`). Linked patients can list, grant, and revoke their own. The seed gives PAT-002391 consent for FMC Abuja so the demo stays ALLOW 8. Break-glass never needs consent. UI: consent panel on `/patients/[publicId]`, consents table on `/security`, patient dashboard grant/revoke.
...
- **INN-70** log out everywhere — `users.sessionsInvalidatedAt` is stamped on password reset and password change. Any session with `createdAt` earlier than that stamp is rejected immediately, even if the paged delete has not finished. `changePassword` issues a fresh session so the caller stays signed in (documented on account settings, INN-86).
- **INN-71** behavioural risk — `scoreAccessRequest` still additive. Location is the source-facility city (mismatch adds points); prior requests in 24 hours vs `normalPatientVolume` add points; clinical after-hours is explained with 0 points. Device is not scored. Ibrahim treatment stays ALLOW 8; harvest stays BLOCK 94.
- **INN-75** decision-engine measurement — `scoreAccessRequest` p95 vs the 1-second NFR-04 **target** (not a production SLO). Write-up: `docs/features/INN-75-decision-latency-evaluation/EVALUATION.md`. `npx vitest run convex/decisionEvaluation.test.ts`
- **INN-76** judges dry-run — after Ibrahim's seven steps, README covers VERIFY/step-up, the `/security` queue, Chioma's portal, and `/facilities`.
- **INN-77** patient-owned consent — linked patients (`users.patientId`) list, grant, and revoke their own consents (`listMyConsents` / `grantMyConsent` / `revokeMyConsent`). Audited as `ConsentRecorded` / `ConsentRevoked` with the patient as actor. Seed still pre-grants FMC Abuja for PAT-002391. Break-glass still needs no consent.
...
- **INN-59** wrapped auth errors — `toUserFacingError` also recovers session/permission wording from a wrapped `[CONVEX M(…)] Uncaught Error: …` string via `findAuthErrorMessage`. List queries still swallow auth failures with `isAuthAppError` codes.
...
- **INN-67** seed audit — `seedAccessEvent` writes history only through `appendAuditEvent`. Seeded `SecurityAlertRaised` rows use the alert id as `entityId`, matching the live alert service.
- **INN-69** discovery audit — opening a patient detail page writes one `PatientDiscovered` row per session and publicId (hit or miss). The live `getPatientDiscovery` query stays a query; `recordPatientDiscovery` is the write.
