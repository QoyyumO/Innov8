...
- **INN-36** patient discovery — `convex/patients.ts`, `/patients`, `/patients/[publicId]`. Demo step 2 works: identity + record existence only, audited. Every `searchPatients` call with a non-empty query writes `PatientSearched`, including zero hits (`resultCount: 0`) so ID enumeration is visible (INN-56).
...
**Next:** the MVP demo path is complete. Should-have work: none open. Follow-ups: none open. Do **not** revive canceled tickets INN-5–INN-17 or INN-34; file new Innov8 issues. Check Linear for the current assignee before starting a ticket.
