...
- **INN-36** patient discovery — `convex/patients.ts`, `/patients`, `/patients/[publicId]`. Demo step 2 works: identity + record existence only, audited. Every `searchPatients` call that reaches an index writes `PatientSearched`, including zero hits (`resultCount: 0`) so ID enumeration is visible (INN-56). A query rejected before any lookup — empty, or a name prefix under `NAME_PREFIX_MIN_LENGTH` — is not a search and is not audited; `isSearchableQuery` in `convex/lib/searchLimits.ts` is the single definition of that line, shared by the service, the mutation, and the search form.
