...
type TextareaProps = {
  id?: string; // Links the field to a <Label htmlFor>
...
const TextArea: React.FC<TextareaProps> = ({
  ...
  if (disabled) {
    textareaClasses +=
      ' bg-gray-100 opacity-50 text-gray-800 border-gray-300 cursor-not-allowed dark:bg-gray-800 dark:text-gray-700 dark:border-gray-700';
  } else if (error) {
    textareaClasses +=
      ...
      {hint && (
        <p
          className={`mt-2 text-sm ${
            error ? 'text-error-500' : 'text-gray-800 dark:text-gray-700'
          }`}
        >
          {hint}
        </p>
