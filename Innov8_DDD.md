...
- **INN-40** — `RecordExchangeService` (`convex/lib/services/recordExchangeService.ts`) + `convex/records.ts`: permitted fields only, target facility only, after ALLOW or a live emergency grant; `RecordViewed` audited. `lab_results` added in INN-80.
...
- **INN-42** — AuditEvent read model (`convex/audit.ts`): role-scoped, paginated, newest-first by `createdAt`; still append-only (no update/delete path). Logout, password change, password reset, and profile update are audited (INN-58). Failed login is audited as `UserLoginFailed` (INN-72). Reviewers filter by patient, facility, decision outcome, and date (INN-78).
...
- **INN-59** — Client `toUserFacingError` still extracts session-expired and permission-denied wording from a wrapped Convex `Error` string when `error.data` is missing.
...
- **RecordType**: `medical_summary` | `allergies` | `medications` | `diagnoses` | `lab_results`
- **RiskBreakdown**: `{ score: 0-100, outcome, reasons: string[], factors: { role, purpose, sameHospital, recordCount, consent, locationMismatch, afterHours, recentRequestCount } }` — ALLOW < 40, VERIFY 40–79, BLOCK ≥ 80
