...
| 2. Search PAT-002391 | Done — `/patients` and `/patients/[publicId]` (clinicians only); identity + record existence, no clinical contents; `PatientSearched` audit on every search that reaches an index, including zero-result ones, so ID enumeration leaves a trail |
