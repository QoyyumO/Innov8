...
| Step | Status |
| --- | --- |
| 1. Authenticate | Done — session login (failed passwords return a form error, not an overlay, and write `UserLoginFailed`), 30-minute sessions or 7 days with Keep me logged in, role-filtered sidebar (patients do not see break-glass or security), `UserLoggedIn` audit. A session deletes itself when it expires, so an open tab stops showing records at that moment rather than when something else happens to refresh it. Password change stamps `sessionsInvalidatedAt` and **issues a fresh session so this tab stays signed in**; other sessions die immediately (INN-86 / INN-70). Password **reset** is the log-out-everywhere path (no new token). Demo accounts exist only after seed; login does not insert users. |
| 2. Search PAT-002391 | Done — `/patients` and `/patients/[publicId]` (clinicians only); identity + record existence, no clinical contents. Public IDs are **case-insensitive**: `PAT-002391`, `pat-002391`, and mixed case all return Chioma (INN-82). `PatientSearched` audit on every search that reaches an index, including zero-result ones; opening a public ID writes one `PatientDiscovered` (including misses), so ID enumeration is visible |
| 3–4. Purpose request + risk decision | Done — `/requests/new` (or **Request access** on a patient page): purpose + record types (role-limited in INN-79) → stored request, risk score (INN-38 / INN-71), ALLOW / VERIFY / BLOCK with every reason (location mismatch and 24-hour request volume vs the worker baseline are extra explainable points; device is not scored; night treatment stays 8). `/requests` lists your requests; `AccessRequested` + outcome audit. Ibrahim → PAT-002391 treatment = 8 ALLOW (the seed gives PAT-002391 consent for FMC Abuja). Cross-facility single-patient requests need **patient consent** (except purpose emergency): without it they are VERIFY with a "no active patient consent" reason; clinicians record consent on the patient page (30 days, audited), the patient can grant or revoke it on their dashboard (INN-77), and security officers / admins can revoke it on `/security`. A VERIFY request opens a **Complete verification** dialog: re-entering your password turns it into ALLOW; three wrong passwords block it and alert security (all audited). Validation failures (unknown patient, records not held, short justification, and the rest) throw a `ConvexError` with a typed `code`, so the same wording reaches the form in a production deploy. Engine-only latency vs the 1-second NFR-04 **target** is in [`docs/features/INN-75-decision-latency-evaluation/EVALUATION.md`](docs/features/INN-75-decision-latency-evaluation/EVALUATION.md) (not a production SLO) |
| 5. Authorised summary | Done — on an allowed request, **View authorised records** releases only the requested sections from the facility that holds them (e.g. FMC Lagos for PAT-002391, including synthetic **lab results** when requested — INN-80); BLOCK / VERIFY release nothing; each view is audited as `RecordViewed`. Allowed access lasts **24 hours** from the decision; after that the request page shows **Request access again**, and any attempt to open the records is refused and audited as `AccessExpired`. After ALLOW, the requesting **doctor** can append a short synthetic note (INN-81, audited `ClinicalNoteAppended`) — not a hospital chart editor |
| 6. Harvest BLOCK + alert | Done — **Simulate bulk harvest (500 records)** on `/requests` calls `simulateBulkHarvest` (the server fixes the count) → BLOCK 94; every BLOCK raises a high-severity alert (`SecurityAlertRaised` audit); security officers and admins review, acknowledge, and close alerts on `/security` (Open / Acknowledged / Closed tabs order by event `createdAt`, same as All); each acknowledge / close is audited with the officer's name |
| 7. Break-glass | Done — `/emergency` (or **Use break-glass** on a blocked or challenged request): a clinician gives a written justification and confirms; the server grants **15 minutes** of access (the client cannot choose the length). The Track C deep dive §8 example was 30 minutes; this MVP uses 15. The grant raises a medium alert and audits `EmergencyGranted`. Records open on the request page while the grant is live; it ends by scheduled expiry (`EmergencyExpired`) or when the holder or a security officer ends it early (`EmergencyRevoked`) |
| Audit trail | Done — `/audit` lists audit events newest first, 25 at a time, with an action filter. Reviewers can also filter by patient ID, facility, decision (ALLOW/VERIFY/BLOCK), and Lagos date (INN-78). Sign-out, password change, password reset, profile update, and failed sign-in are audited. Clinicians and patients see only their own activity; security officers and system admins see everyone's; hospital admins see their own staff plus every request to or from their facility. Reviewers can open one person's trail. Read-only: nothing edits or deletes audit rows |
| Live dashboards + facilities | Done — every role dashboard reads live, bounded data. Clinicians (doctor, nurse, pharmacist, laboratory) see their requests today by outcome, last decision, latest blocked harvest, active break-glass, and recent requests. Security officers and system admins see open alerts by severity, blocks today, active break-glass, audit volume today, the live alert queue, and the latest decisions; hospital admins see the same summary for their facility plus the facility list. `/facilities` lists the participating hospitals with their stored worker and patient totals. "Today" means since midnight in Lagos. Chioma's patient dashboard shows PAT-002391 identity, home facility, who may request her records (grant/revoke consent), and a bounded history of audit events that name her (after seed links `users.patientId`) |

### Judges dry-run (after the seven Ibrahim steps)

The table above is the Ibrahim-only Track C pass (ALLOW 8 → view → harvest 94 → break-glass). Before a judging session, walk these extra screens so VERIFY, the security queue, the patient portal, and `/facilities` are not skipped. Password for every demo account is still `password123`.

1. **VERIFY + step-up (INN-44 / INN-45).** Seeded consent exists only for PAT-002391 → FMC Abuja, which is why the Ibrahim walkthrough scored ALLOW 8. Sign out. Sign in as `security@innov8.ng`, open `/security`, scroll to **Patient consents**, and **Revoke** the PAT-002391 row for FMC Abuja. Sign in as `ibrahim@fmc.abuja.ng`, open `/patients/PAT-002391`, and request **Treatment** again → **VERIFY** with a “no active patient consent” reason. **Complete verification** stays refused until a clinician records consent on that patient page (30-day note); a password cannot stand in for consent, and those refused attempts do not decrement remaining tries (INN-84). Record consent, then re-enter `password123` in the dialog → **ALLOW**. Wrong passwords after consent decrement remaining attempts; the 3rd failure BLOCKs and alerts. Expected-results list: [`docs/qa/TEST-FINDINGS.md`](docs/qa/TEST-FINDINGS.md).
2. **Security officer (INN-39 / INN-43).** Sign out. Sign in as `security@innov8.ng` again. Open `/security`. In **Security alerts**, acknowledge the **harvest** alert from step 6 and the **break-glass** alert from step 7 (Open → Acknowledged). Each action is audited. Do not confuse this with the consents table used in step 1.
3. **Patient portal (INN-46 / INN-77).** Sign out. Sign in as `chioma@patient.innov8.ng`. The home dashboard shows PAT-002391 identity, home facility (FMC Lagos), access history, and **Who may request your records** (grant/revoke). She can revoke the seeded FMC Abuja consent herself; the VERIFY walkthrough in step 1 still works via `/security`. Clinician accounts do not see this view.
4. **Facilities (INN-68).** Sign out. Sign in as `security@innov8.ng` or `admin@fmc.abuja.ng` and open `/facilities` from the sidebar. That link is hidden from clinicians on purpose; do not use a doctor account for this beat.
