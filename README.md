...
1. **VERIFY + step-up (INN-44 / INN-45).** Seeded consent exists only for PAT-002391 → FMC Abuja, which is why the Ibrahim walkthrough scored ALLOW 8. For VERIFY, sign in as `security@innov8.ng`, open `/security`, and **Revoke** that consent. Sign back in as Ibrahim, open PAT-002391, and request **Treatment** again → **VERIFY** with a “no active patient consent” reason. **Complete verification** stays refused until a clinician records consent on that patient page (30-day note). Record consent, then re-enter `password123` in the dialog → **ALLOW**. A password cannot stand in for consent.
2. **Security officer (INN-39 / INN-43).** Stay on (or sign back in as) `security@innov8.ng`. Open `/security`. Acknowledge the **harvest** alert from step 6 and the **break-glass** alert from step 7 (Open → Acknowledged). Each action is audited.
...
4. **Facilities (INN-68).** Sign in as `security@innov8.ng` or `admin@fmc.abuja.ng` and open `/facilities`. The sidebar hides that link from clinicians on purpose; do not use a doctor account for this beat.
