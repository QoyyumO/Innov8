...
| Step | Status |
| --- | --- |
| 1. Authenticate | Done — session login (failed passwords return a form error, not an overlay), 30-minute sessions or 7 days with Keep me logged in, role-aware sidebar, `UserLoggedIn` audit. A session deletes itself when it expires, so an open tab stops showing records at that moment rather than when something else happens to refresh it. Demo accounts exist only after seed; login does not insert users. |
